...
func (conn *Conn) DescribeForce(number uint64) (desc *Description, err error) {
	var (
		results []Result
	)
	if results, err = conn.RunMarshaled("describe", []string{"-f", strconv.FormatUint(number, 10)}); err != nil {
		return
	}
	for idx := range results {
		if d, ok := results[idx].(*Description); !ok {
